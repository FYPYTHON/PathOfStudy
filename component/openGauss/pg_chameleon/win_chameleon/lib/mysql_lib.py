...
class mysql_source(object):
    ...
    def init_workers(self, count, target_func, pool, worker_prefix):
        ...
        for x in range(count):
            ...
            process = Thread(target=target_func)
            process.daemon = True
    ...
    def __copy_tables(self):
        ...
        # self.write_task_queue = multiprocessing.Manager().Queue(1)
        # self.read_task_queue = multiprocessing.Manager().Queue(1)
        # self.index_waiting_queue = multiprocessing.Manager().Queue(1)
