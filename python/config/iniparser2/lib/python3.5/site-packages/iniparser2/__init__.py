...
class ParsingError(Exception):
    ...
    def __str__(self):
        return "{}, {} [line {}]".format(self.message, self.text, self.line)
    ...
    def update(self, dict_: dict):
        ...
        for sect in dict_:
            if sect in self._sections and type(dict_[sect]) is not dict:
                ...
                raise SectionError("Cannot update section header value [{}]".format(sect))
            ...
            if isinstance(dict_[sect], dict):
                ...
                for opt in dict_[sect]:
                    if (
                        ...
                        # )
                        raise PropertyError(
                            "value must be a literal or NoneType [{}][{}]".format(sect, opt)
                        )
                ...
                if (
                    ...
                    raise PropertyError("value must be a literal or NoneType [{}]".format(sect))
    ...
    def remove_property(self, name: str, section: str = None) -> None:
        if section is None:
            ...
            if not self.has_property(name, section):
                ...
                raise PropertyError("property {} not found in section {}".format(name, section))
